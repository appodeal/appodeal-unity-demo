...
{
    private readonly static string JNI_SAFE_PATH =
        AppodealUnityUtils.combinePaths(Application.dataPath, "Appodeal", "InternalResources", "jni");

    private static Dictionary<AppodealUnityUtils.AndroidArchitecture, string> archDict =
    ...
    private static void setSelectedArchitectures(AppodealUnityUtils.AndroidArchitecture arch)
    {
        ...
        HashSet<AppodealUnityUtils.AndroidArchitecture> selectedArches =
        ...
        foreach (string libPath in nativeLibs)
        {
            ...
            Dictionary<AppodealUnityUtils.AndroidArchitecture, string> archFullPaths =
                new Dictionary<AppodealUnityUtils.AndroidArchitecture, string>();
            Dictionary<AppodealUnityUtils.AndroidArchitecture, string> archSafeFullPaths =
                new Dictionary<AppodealUnityUtils.AndroidArchitecture, string>();
            HashSet<AppodealUnityUtils.AndroidArchitecture> presentedArches =
                new HashSet<AppodealUnityUtils.AndroidArchitecture>();
            HashSet<AppodealUnityUtils.AndroidArchitecture> savedArches =
                new HashSet<AppodealUnityUtils.AndroidArchitecture>();
            foreach (AppodealUnityUtils.AndroidArchitecture a in Enum.GetValues(
                typeof(AppodealUnityUtils.AndroidArchitecture)))
            {
                ...
                typeof(AppodealUnityUtils.AndroidArchitecture)))
            {
                ...
                if (selectedArches.Contains(a) != presentedArches.Contains(a))
                {
                    ...
                    else
                    {
                        ...
                        {
                            string message = "Plugin " + dir + " doesn't support the following architecture: " +
                                             archDict[a] +
        ...
        if (!dialogWasShown)
            EditorUtility.DisplayDialog("Appodeal Notification", "Filtering finished. All possible problems resolved.",
                "Ok");
    }
}